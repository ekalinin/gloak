...
- the device authorization grant **end to end**, including the verification
  page, the consent page and a login a user can actually complete - and that
  `expired_token` is a fifteen-second window rather than a state. CIBA's
  refusals too, whose flow **cannot** be measured on a default 26.7.1 because
...
- a parity meter whose denominator comes from Keycloak's own OpenAPI description
  rather than from a hand-kept list: **598 of 660 enumerated behaviours served**,
  plus one chapter whose surface has not been counted
...
The **management port** is enumerated and served by nothing: all sixteen of its
behaviours are measured and recorded, because Gloak has no second listener at
all. A default Keycloak has none either - port 9000 comes up only under
...
| `POST /realms/{realm}/protocol/openid-connect/auth/device` | device authorization |
| `GET /realms/{realm}/device` | the device verification page |
| `POST /realms/{realm}/login-actions/consent` | the consent decision |
...
```
total: 598 of 660 enumerated behaviours served; 1 chapters not enumerated
```

The denominator is 660 rather than 413 plus a fixed number because the protocol
...
had five, and again on 2026-09-07 when a chapter that had never had a
denominator got one: SAML is enumerated by hand against a running server, +18.
The Admin API chapters cannot move this way and none has.
...
underneath it: one chapter has no denominator, so work served in one of them
